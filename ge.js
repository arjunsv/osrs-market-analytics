...
class Chart {
	constructor(divId, titleText, xTitle, priceOrQuantity) {
		...
		addData(color, itemId, itemName) {
			...
			this.orderIdsAdded[itemId] = Object.keys(this.orderIdsAdded).length
			this.chart.data.push(newDataDict);
			this.regenerateChart(this.chart.data);
			this.updateChart(1);
		}
		regenerateChart(datapoints) {
			this.chart = new CanvasJS.Chart(this.divId, {
			zoomEnabled: true,
			title: {
				text: this.titleText,
				fontFamily: "Helvetica Neue",
		...
		updateChart(count) {
			count = count || 1;
			var myself = this;
			function callBack(item, id) {
				console.log("from callBack");
				console.log(myself);
				if (myself.priceOrQuantity) {
					myself.chart.options.data[myself.orderIdsAdded[id]].legendText = itemIdToName[id] + " - QTY TRADED: " + String(item.sellingQuantity);
					myself.chart.data[myself.orderIdsAdded[id]].dataPoints.push({
					x: time.getTime(),
					y: item.sellingQuantity
					});
				} else {
					myself.chart.options.data[myself.orderIdsAdded[id]].legendText = itemIdToName[id] + " - PRICE: " + String(item.selling);
					myself.chart.data[myself.orderIdsAdded[id]].dataPoints.push({
					x: time.getTime(),
					y: item.selling
					});
			...
			console.log("from updateChart");
			console.log(this);
			for (var id in this.orderIdsAdded) {
				httpGet("https://api.rsbuddy.com/grandExchange?a=guidePrice&i=" + id, callBack, id);
			}
			this.chart.render();
			}
		}
	}
 // for the window onload function
...
setInterval(function() { 
				for (var i = 0; i < listOfCharts.length; i++) {
					console.log("listofcharts from setinterval:")
					console.log(listOfCharts)
					listOfCharts[i].updateChart(1);
					updatePopulation();
				}
			}, updateInterval);
